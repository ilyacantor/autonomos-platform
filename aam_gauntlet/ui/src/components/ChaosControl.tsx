...
export default function ChaosControl({ onScenarioRun }: ChaosControlProps) {
  ...
  const handleRunScenario = async () => {
    setIsRunning(true)
    try {
      // Set chaos level on farm
      await apiClient.setChaosLevel(chaosLevel)
      ...
      const result = await apiClient.runScenario(scenario)
  ...
  const handleReset = async () => {
    try {
      await apiClient.resetFarmMetrics()
      await apiClient.setChaosLevel('mild')
      setChaosLevel('mild')
    } catch (error) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <input
            ...
            disabled={isRunning}
          />
          <div className="text-center text-lg font-bold text-blue-400">{duration}s</div>
        </div>
        ...
        <div className="flex flex-col justify-end space-y-2">
          <button
            ...
            className={`px-4 py-2 rounded font-medium flex items-center justify-center space-x-2 ${
          ...
          <button
            onClick={handleReset}
            disabled={isRunning}
            className="px-4 py-2 rounded font-medium bg-gray-700 text-gray-300 hover:bg-gray-600 flex items-center justify-center space-x-2"
          >
            <RotateCcw size={16} />
            <span>Reset</span>
          </button>
