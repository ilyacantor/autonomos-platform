...
if [ $audit_exit_code -eq 0 ]; then
  echo "✅ GUARD PASSED"
  echo ""
  echo "Database schemas are synchronized and tracked by Alembic."
  ...
  echo "  2. If prod lacks alembic_version, run: ./scripts/stamp_prod_baseline.sh"
  echo "  3. If schemas diverged, investigate the differences manually"
  echo "  4. Re-run this guard script after fixes"
  echo "  5. Only then proceed with Replit Publishing"
  echo ""
