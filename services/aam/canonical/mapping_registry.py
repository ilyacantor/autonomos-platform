...
class MappingRegistry:
    ...
    All mappings are loaded from YAML/JSON files.
    """
    ...
    def __init__(
        ...
        logger.info("MappingRegistry initialized with YAML/JSON storage")
    ...
    def _load_all_mappings(self):
        ...
        logger.info(f"Loaded {len(self._mappings_cache)} YAML/JSON mappings")

    def _get_mapping_from_cache(self, system: str, entity: str) -> Optional[Dict[str, Any]]:
    ...
    def get_mapping(self, system: str, entity: str, tenant_id: str = "default") -> Optional[Dict[str, Any]]:
        ...
        Loads from YAML/JSON cache.
        """
        logger.debug(f"Getting mapping for {system}.{entity}")
        return self._get_mapping_from_cache(system, entity)

    async def get_mapping_async(self, system: str, entity: str, tenant_id: str = "default") -> Optional[Dict[str, Any]]:
        """
        Get field mapping for a system and entity (async version).
        Loads from YAML/JSON cache.
        """
        logger.debug(f"Getting mapping for {system}.{entity} (async)")
        return self._get_mapping_from_cache(system, entity)
