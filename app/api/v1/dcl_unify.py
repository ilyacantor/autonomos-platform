...
async def run_unification(
    ...
    Algorithm:
    1. Query all contacts from canonical_streams with non-null emails
    2. Group by LOWER(TRIM(email))
    3. For each email group:
       - Find or create unified_contact record
    ...
    """
    try:
        logger.info(f"Starting DCL contact unification for tenant: {current_user.tenant_id}")
        ...
        logger.info(f"Found {len(contacts)} contacts with emails")
        ...
        for email, contact_list in email_groups.items():
            # Check if unified contact already exists
            ...
            if existing_unified:
                ...
                logger.debug(f"Reusing existing unified contact {unified_contact_id} for {email}")
            else:
                ...
                unified_count += 1
                logger.debug(f"Created new unified contact {unified_contact_id} for {email}")
            ...
            for contact in contact_list:
                ...
                if not existing_link:
                    ...
                    db.add(new_link)
                    link_count += 1
                    logger.debug(f"Created link: {source_system}:{contact.contact_id} -> {unified_contact_id}")
                else:
                    ...
                    if existing_link.unified_contact_id != unified_contact_id:
                        ...
                        logger.debug(f"Updated link: {source_system}:{contact.contact_id} -> {unified_contact_id}")
        
        # Commit all changes
        db.commit()
        
        logger.info(f"Unification complete: {unified_count} new unified contacts, {link_count} new links")
