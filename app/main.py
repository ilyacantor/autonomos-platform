...
# Enterprise Agent Requirements: Certification, A2A Protocol, Horizontal Scaling
...
if os.path.exists(STATIC_DIR) and os.path.isdir(STATIC_DIR):
    ...
    @app.get("/scheduler")
    def serve_scheduler(request: Request):
        """Serve Scheduler frontend page"""
        index_path = os.path.join(STATIC_DIR, "index.html")
        if os.path.exists(index_path):
            return FileResponse(
                index_path,
    ...
    def serve_orchestration(request: Request):
        """Serve Orchestration Dashboard frontend page"""
        index_path = os.path.join(STATIC_DIR, "index.html")
        if os.path.exists(index_path):
            return FileResponse(
                index_path,
    ...
    def serve_guide(request: Request):
        """Serve Platform Guide frontend page"""
        index_path = os.path.join(STATIC_DIR, "index.html")
        if os.path.exists(index_path):
            return FileResponse(
                index_path,
