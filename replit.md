...
- **DCL Graph Container Overflow Fix (Nov 21, 2025)**: Fixed fundamental issue where graph content (nodes + labels) was bleeding outside container bounds. Root cause: viewBox calculation only considered node positions but labels extended beyond those bounds. Solution: Calculate true content extent by measuring label widths during layout phase, then use complete bounds (contentMaxX) for viewBox calculation. SVG's preserveAspectRatio="xMidYMid meet" now properly scales entire graph to fit container without overflow. No hard-coded dimensions - fully responsive and organic solution.
- **DCL Graph Filtering Fix (Nov 21, 2025)**: Fixed disabled filtering logic in DeterministicSankeyGraph that was showing all nodes regardless of user selections. Restored proper source and agent filtering - graph now dynamically updates when users check/uncheck data sources or agents. WebSocket connection also fixed to use window.location.host instead of localhost for proper connectivity. Added labels to L1 source nodes displaying their data source system names (Salesforce, MongoDB, etc.) with color-coded pillbox styling.
- **Embedded Replit Applications (Nov 21, 2025)**: Replaced in-app Discovery Demo with embedded external Replit applications using simple iframe pattern (matching AOD implementation). New **AOS Overview** page embeds discovery-demo-standalone app for interactive demos. **AAM Connect** simplified to metrics + embedded autonomos-mesh app. Both use aspect-ratio container (paddingBottom: 75%) with basic iframe - no complex loading states or sandbox restrictions for faster load times. Removed DiscoveryDemoPage and EmbeddedApp components (bundle reduced ~150KB JS + 8.5KB CSS). Navigation updated: Platform Guide → AOS Overview → Control Center → Discovery → Connections → Ontology → Orchestration → Agents → Help.
- **DCL Sankey Graph Rendering Fix (Nov 21, 2025)**: Fixed critical d3-sankey "missing: 189" error and layer positioning issues. **Two-part fundamental fix**: (1) Changed `SankeyLink` source/target from `number` to `number | string`, updated link generation to use actual node IDs (`e.source`, `e.target`) instead of array indices (`nodeIndexMap[e.source]`) - resolves "missing: 189" error. (2) Discovered d3-sankey **overwrites `depth` property** during breadth-first layout calculation and `.nodeAlign()` only controls vertical positioning within layers, NOT layer assignment. **Root cause solution**: Use d3-sankey's native `node.layer` property which is respected automatically during layout calculation - no custom nodeAlign callback needed. Graph now uses proper 4-column layout: L0 (1 source_parent) → L1 (~211 sources) → L2 (6 ontology) → L3 (2 agents).
...
**UI/UX Decisions:**
The frontend uses React 18 and TypeScript with a responsive design, Quicksand typography, a Green, Blue, Purple color scheme, and dark mode. It features pages for platform guide, AOS overview (embedded discovery demo), data discovery (AOD), API mesh connections (AAM with embedded mesh app), ontology graph visualization (DCL), agentic orchestration (AOA), and control center. Live status indicators (green pulsing dots) distinguish real backend data from mock data, managed via a centralized registry. Page titles follow "AOS [Feature]" branding. Enterprise reporting views include a tabular lineage grid, hierarchical tree view, and an evaluation dashboard. **Embedded Apps**: Simple iframe pattern using aspect-ratio containers (paddingBottom: 75%) for external Replit applications (discovery-demo-standalone, autonomos-mesh) - matching AOD implementation for consistent, fast loading without complex state management.
