...
The dashboard features a hero section showcasing the product value proposition, followed by the AAM (Adaptive API Mesh) integration layer, and the DCL (Data Connectivity Layer) graph visualization for data mapping and orchestration. The DCL section includes a large Sankey graph with centered descriptive text overlay, and a right sidebar containing the Intelligent Mapping panel, Narration panel, RAG Learning Engine panel, and Intelligence Review panel.
