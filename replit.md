...
- **Embedded Replit Applications (Nov 21, 2025)**: Replaced in-app Discovery Demo with embedded external Replit applications. Created reusable `EmbeddedApp` component with security-hardened iframe sandbox (`allow-scripts allow-forms allow-popups` only), responsive height (500px → 600px → 700px), lazy loading, scoped retry (iframe-only reload via key prop), and error handling. New **AOS Overview** page embeds discovery-demo-standalone app for interactive demos. **AAM Connect** simplified to metrics + embedded autonomos-mesh app. Removed DiscoveryDemoPage (bundle reduced ~150KB JS + 8.5KB CSS). Navigation updated: Platform Guide → AOS Overview → Control Center → Discovery → Connections → Ontology → Orchestration → Agents → Help.
...
**UI/UX Decisions:**
The frontend uses React 18 and TypeScript with a responsive design, Quicksand typography, a Green, Blue, Purple color scheme, and dark mode. It features pages for platform guide, AOS overview (embedded discovery demo), data discovery (AOD), API mesh connections (AAM with embedded mesh app), ontology graph visualization (DCL), agentic orchestration (AOA), and control center. Live status indicators (green pulsing dots) distinguish real backend data from mock data, managed via a centralized registry. Page titles follow "AOS [Feature]" branding. Enterprise reporting views include a tabular lineage grid, hierarchical tree view, and an evaluation dashboard. **Embedded Apps**: Reusable `EmbeddedApp` component provides security-hardened iframes (restricted sandbox), responsive heights, lazy loading, scoped retry, and error handling for external Replit applications (discovery-demo-standalone, autonomos-mesh).
