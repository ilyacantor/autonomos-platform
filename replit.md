...
*   **USE_AAM_AS_SOURCE** (default: `false`): Backend feature flag controlling DCL data source mode. When `false`, uses Legacy CSV file sources (9 demo files). When `true`, uses AAM production connectors (4 sources: Salesforce, Supabase, MongoDB, FilesSource). **Production-Grade Implementation (Nov 2025):** Redis-backed flag storage with multi-worker support, async pub/sub broadcasting, and restart persistence. Precedence model: ENV variable > Redis stored value > hardcoded default. Toggle endpoint: POST `/dcl/toggle_aam_mode` triggers automatic cache invalidation across all workers via `dcl:feature_flags` pub/sub channel. Frontend toggle button automatically triggers fresh `/connect` run with updated sources. Async Redis client (`redis.asyncio`) prevents event loop blocking, ensuring production-ready performance with multiple workers.
