...
*   **AAM Auto-Onboarding with 90% SLO (Nov 8):** Implemented production-ready auto-onboarding system that accepts ConnectionIntent payloads from AOD discovery and onboards data sources in Safe Mode by default. Database schema extended with 7 fields (namespace, first_sync_rows, latency_ms, credential_locator, risk_level, evidence, owner) via Alembic migration `a4e46593d477`. Created ConnectionIntent Pydantic schemas (16 models), OnboardingService with 30+ source type allowlist validation, Redis-backed FunnelMetricsTracker with namespace isolation, and 4 API endpoints (POST /connections/onboard, POST /connections/onboard/batch, GET /metrics/funnel, GET /connections/health). SLO enforcement: GET /metrics/funnel returns 503 with English summary when coverage < 90%, listing top blockers and recommendations. Safe Mode guardrails include idempotent operations, credential validation, and risk-based approval. Includes exec_summary.py script and 5 sample seed files (Salesforce, Slack, GWorkspace, Snowflake, Supabase) for testing. Namespace isolation ensures demo namespace remains untouched.
*   **AOS NLP Gateway - Production RAG Service (Nov 8):** Built complete natural-language gateway service in `services/nlp-gateway/` with 7 REST endpoints (FinOps, RevOps, AOD, AAM, KB search/ingest, Feedback), tenant-scoped RAG knowledge base using Postgres + pgvector, hybrid retrieval (BM25 + vector embeddings with Reciprocal Rank Fusion), JWT auth integration, PII redaction, structured logging with trace_id, E2E tests, and Replit Agent configuration. Architecture: FastAPI async service, async SQLAlchemy models, optional ML dependencies (sentence-transformers, presidio) with graceful fallbacks, Make targets for run/test/lint/eval, 25-prompt eval dataset, HTTP examples, demo data seed script. Database: 5 new tables (kb_documents, kb_chunks with 384-dim vectors, kb_metadata, kb_ingest_jobs, kb_feedback) via Alembic migration `b15b4a5021b3`. Service runs independently on port 8001, ready for MCP wrapping (Phase 2). Designed for Replit Agent integration with contract-first, MCP-friendly schemas.
