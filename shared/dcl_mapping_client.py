...
class AsyncDCLMappingClient:
    """Async client for calling DCL mapping registry API (no event loop blocking)"""
    
    def __init__(self, base_url: Optional[str] = None, timeout: float = 5.0, http_client: Optional[httpx.AsyncClient] = None):
        """
        Initialize async DCL mapping client
        
        Args:
            base_url: DCL API base URL (defaults to localhost:5000 or DCL_API_URL env var)
            timeout: HTTP request timeout in seconds
            http_client: Optional httpx.AsyncClient for dependency injection (e.g., for testing with ASGITransport)
        """
        self.base_url: str = base_url or os.getenv("DCL_API_URL", "http://localhost:5000")
        self.timeout = timeout
        # Note: Redis caching will be added later with async Redis client
        self.redis = None  # TODO: Add async Redis support via redis.asyncio
        ...
        logger.info(f"AsyncDCLMappingClient initialized with base_url={self.base_url}")
    
    async def get_entity_mapping(self, connector: str, entity: str, tenant_id: str = "default") -> Optional[Dict[str, Any]]:
        """
        Get all field mappings for a connector and entity (async, non-blocking).
        ...
        Returns:
            Mapping dictionary or None if not found
        """
        # TODO: Add async Redis caching support
        ...
        try:
            ...
            logger.debug(f"DCL API request (async): GET {url}?source_table={normalized_entity}")
            
            # Use await for async HTTP call - no event loop blocking
            ...
            result = {"fields": fields}
            # TODO: Cache result with async Redis
    ...
    async def get_field_mapping(self, connector: str, entity: str, field: str, tenant_id: str = "default") -> Optional[Dict[str, Any]]:
        """
        Get single field mapping (async, for future use)
